...
# Water Dashboard
    ...
    if selected_ports and not filtered_df.empty:
        ...
        if valid_ports:
            ...
            # Adding geographic locations of ports (latitude and longitude)
            port_locations = {
                "charleston_sc": {"lat": 32.7765, "lon": -79.9311},
                "houston_tx": {"lat": 29.7633, "lon": -95.3633},
                "long_beach_ca": {"lat": 33.7542, "lon": -118.1967},
                "los_angeles_ca": {"lat": 33.742, "lon": -118.2719},
                "nwsa_seattle_tacoma_wa": {"lat": 47.6097, "lon": -122.3331},
                "oakland_ca": {"lat": 37.8044, "lon": -122.2712},
                "port_of_ny_nj": {"lat": 40.6895, "lon": -74.0455},
                "port_of_virginia_va": {"lat": 36.8508, "lon": -76.2859},
                "savannah_ga": {"lat": 32.0809, "lon": -81.0912}
            }

            # Merge the bubble data with port locations
            bubble_data['lat'] = bubble_data['Port'].apply(lambda x: port_locations[x]["lat"])
            bubble_data['lon'] = bubble_data['Port'].apply(lambda x: port_locations[x]["lon"])

            # Create a geographical bubble map
            map_chart = px.scatter_mapbox(
                bubble_data, 
                lat="lat", lon="lon", size="Total Volume", color="Port", 
                size_max=60, zoom=3,
                mapbox_style="open-street-map",
                title="Geographical Bubble Map of Container Volumes by Port"
            )
